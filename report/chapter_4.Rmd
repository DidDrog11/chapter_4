...
We considered a single-mode contact model, direct and indirect contacts were conflated given evidence that the pathogen of interest is transmitted by both means [@safronetz_temporal_2022]. We fit the following model to the data where all effects are relative to no contact (Equation 2).
...
To model contacts between individual small mammal species we produce a buffer zone, with a radius of 50m, around each trapped individual. Rodents detected within this area during the same trapping visit are considered as potential contacts. The `sf` package in the R statistical computing language was used for geospatial manipulation and analysis with the `igraph` package used to produce the rodent networks [@pebesma_simple_2018; @r_core_team_r_2021]. To test whether rodent contact rates vary by landuse type we analyse the composition of these networks comparing edge density, betweenness and .... Within these networks we further explore species level measures of degree, closeness and betweenness.
...
Each rodent has only a single observation, the probability of an inferred contact being a true contact is therefore impossible to estimate. For example, if both rodents were trapped at the periphery of their individual-level space use the probability of a direct or indirect encounter would be low, compared to if both were trapped at the core of their space use.
